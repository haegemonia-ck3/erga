...
export const toolDefinitions: AgentToolParam[] = [
  ...
  { type: 'function', name: 'bulk_update_issues', description: 'Update 1–50 distinct issues in one repository in a single tool call. Each update specifies its issue number and only the requested fields. Prefer this over individual calls for multiple assignments, milestones, labels, closing or reopening issues, titles or bodies. Assignees and labels REPLACE lists: inspect and preserve existing values first. No confirmation. Returns per-issue applied/failed/unknown/skipped results; stops on first error. Not atomic: earlier changes remain applied. Never retry applied or unknown items.', parameters: schema(bulkIssueUpdateSchema) },
